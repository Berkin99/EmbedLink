...
//         targetAtt[i]  = (crange.axis[i]) - (kinematicsState()->rotation.axis[i] / ANGLE_MAX);
//         targetAtt[i] *= ANGLERATE_MAX;

//         /* PID Handle Update */
//         hpidAtt[i].dt = 1;
//         att[i] = (kinematicsState()->iattitude.axis[i] * 0.2) + (att[i] * 0.8);
//         pidOut[i] = pidUpdate(&hpidAtt[i], att[i], targetAtt[i]);
//         pidOut[i] = constrainFloat(pidOut[i] , -pidAttLimit[i], pidAttLimit[i]);
//     }

//     /* Motor Power Calculation */
//     cpow   = constrainFloat(cpow, 0, 1);
...
//         mout.m[i] = constrainFloat(mout.m[i], 100, 1000); /* Keep Motors Running */
//         mout.m[i] /= 1000.0f;
//     }
