...
static void madgwickUpdateQImpl(float gx, float gy, float gz, float ax, float ay, float az, float dt);
...
{
  madgwickUpdateQImpl(gx, gy, gz, ax, ay, az, dt);
...
void madgwickGetQuaternion(float* q_x, float* q_y, float* q_z, float* q_w)
{
  *q_x = qx;
  *q_y = qy;
  *q_z = qz;
...
{
  return madgwickGetAccZ(ax, ay, az) - baseZacc;
}
